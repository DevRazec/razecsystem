...
namespace razecsystem.Tests
{
    public abstract class razecsystemTestBase : AbpIntegratedTestBase<razecsystemTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected razecsystemTestBase()
        {
        ...
        protected void UsingDbContext(Action<razecsystemDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<razecsystemDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<razecsystemDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<razecsystemDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<razecsystemDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<razecsystemDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<razecsystemDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<razecsystemDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<razecsystemDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<razecsystemDbContext, Task<T>> func)
        {
            T result;
